...
use crate::config_parser::{ParseError, Token, tokenize};
...
impl ConfigParser {
    ...
    fn parse_servers(&mut self) -> Result<Vec<ServerConfig>, ParseError> {
        ...
        if !servers.is_empty() {
            ...
            Err(ParseError::new(
                0,
                "Not a single valid server configuration found".to_string(),
            ))
        }
    }
    ...
    fn parse_server(&mut self) -> Result<ServerConfig, ParseError> {
        ...
        while self.current() != Some(&Token::CloseBrace) {
            match self.current() {
                Some(Token::Word(w)) => {
                    match w.as_str() {
                        // as_str() is a String method that converts the String to a &str
                        "host" => {
                            // the only reason we do this is to be able to match w (String) tokens with literals like "host", "ports", etc
                            self.advance_parser();
                            ...
                            host = Some(host_str.parse().map_err(|_| {
                                ...
                                ParseError::new(
                                    self.current_line(),
                                    format!("Invalid IP address: {}", host_str),
                                )
                            })?);
                        }
                        "ports" => {
                            self.advance_parser();
                            while let Some(Token::Word(p)) = self.current() {
                                match p.parse::<u16>() {
                                    Ok(port) => {
                                        if ports.contains(&port) {
                                            return Err(ParseError::new(
                                                self.current_line(),
                                                format!("Duplicate port in server block: {}", port),
                                            ));
                                        }
                                        ports.push(port);
                                        self.advance_parser();
                        ...
                        _ => {
                            if w == "server" {
                                return Err(ParseError::new(
                                    self.current_line(),
                                    "Missing closing brace '}' in server block".to_string(),
                                ));
                            }
                            return Err(ParseError::new(
                                self.current_line(),
                                format!("Unknown directive: {}", w),
                            ));
                        }
                    }
                }
        ...
        if host.is_none() {
            return Err(ParseError::new(
                self.current_line(),
                "Missing required 'host' directive",
            ));
        }
        if ports.is_empty() {
            return Err(ParseError::new(
                self.current_line(),
                "Missing required 'ports' directive",
            ));
        }
        if server_name.is_empty() {
            return Err(ParseError::new(
                self.current_line(),
                "Missing required 'server_name' directive",
            ));
        }
    ...
    fn parse_route_body(&mut self) -> Result<RouteConfig, ParseError> {
            ...
            match self.current() {
                Some(Token::Word(w)) => {
                    match w.as_str() {
                        "methods" => {
                            self.advance_parser();
                            while let Some(Token::Word(m)) = self.current() {
                        ...
                        "root" => {
                            // root is required for RouteConfig struct
                            self.advance_parser();
                            root = self.consume_word()?;
                        }
                        "index" => {
                            // index is not required for RouteConfig struct (so it can be None)
                            self.advance_parser();
                        ...
                        "redirect" => {
                            ...
                            let status_code: u16 = status_code_str.parse().map_err(|_| {
                                ParseError::new(
                                    self.current_line(),
                                    format!("Invalid redirect code: {}", status_code_str),
                                )
                            })?;
                        ...
                        _ => {
                            return Err(ParseError::new(
                                self.current_line(),
                                format!("Unknown route directive: {}", w),
                            ));
                        }
                    }
                }
        ...
        if redirect.is_none() && root.is_empty() {
            return Err(ParseError::new(
                self.current_line(),
                "Route must have either 'root' or 'redirect' directive",
            ));
        }
