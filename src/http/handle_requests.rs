...
use crate::config::{Method, RouteConfig, ServerConfig};
    ...
    fn resolve_get_file_path(
        ...
        if rel.is_empty() {
            ...
            let index_file = match route.index_file.as_deref() {
                Some(index) => index,
                None => {
                    return Err(HttpResponseError::new_err_response_with_pages(
                        404,
                        "Not Found",
                        &server.error_pages,
                    ));
    ...
    pub fn handle_get(
        ...
        if route.cookie_required {
            let Some(session_id) = self.session_id.as_ref() else {
                return RouteAction::Immediate(HttpResponseError::new_err_response_with_pages(
                    403,
                    "Forbidden",
                    &server.error_pages,
                ));
        ...
        match std::fs::read(file_path) {
            Ok(contents) => RouteAction::Immediate(
                HttpResponseOk {
                    status_code: 200,
            ...
            Err(_) => RouteAction::Immediate(HttpResponseError::new_err_response_with_pages(
                404,
                "Not Found",
                &server.error_pages,
            )),
...
impl HttpRequest {
    pub fn handle_delete(&self, route: &RouteConfig, server: &ServerConfig) -> Vec<u8> {
        ...
        if !self.path.starts_with(&expected_prefix) {
            return HttpResponseError::new_err_response_with_pages(
                404,
                "Not Found",
                &server.error_pages,
            );
        ...
        if !file_path.exists() {
            ...
            match std::fs::remove_file(file_path) {
                ...
                .response_ok_to_bytes(),
                Err(_) => HttpResponseError::new_err_response_with_pages(
                    500,
...
impl HttpRequest {
    ...
    pub fn execute_route(
        ...
        match self.method.as_str() {
            ...
            _ => RouteAction::Immediate(HttpResponseError::new_err_response_with_pages(
                405,
                "Method Not Allowed",
                &server.error_pages,
            )),
