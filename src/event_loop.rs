use libc::{
    EPOLL_CTL_ADD, EPOLL_CTL_DEL, EPOLLIN, epoll_create1, epoll_ctl, epoll_event, epoll_wait,
};
...
pub fn run(listeners: Vec<TcpListener>, config: &AppConfig) {
    let listener_fds: HashSet<i32> = listeners
        .iter()
    ...
    for listener in &listeners {
        ...
        let event_add_result =
            unsafe { epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listener_fd, &mut listener_event) };
    ...
    loop {
            ...
            if listener_fds.contains(&fd) {
                    ...
                    .accept()
                {
                    ...
                    Ok((client_socket, addr)) => {
                        ...
                        let client_add_result = unsafe {
                            epoll_ctl(epoll_fd, EPOLL_CTL_ADD, client_fd, &mut client_event)
                        };
                        if client_add_result < 0 {
                            eprintln!("Failed to add client to epoll");
                        } else {
                            let port = listeners
                                .iter() // find the listener that accepted this client to
                                .find(|l| l.as_raw_fd() == fd)
                                .unwrap() // get its port number for the Client struct
                                .local_addr()
                                .unwrap()
                                .port();
                        ...
                        Ok(bytes_read) => {
                            ...
                            while let Some(end) = check_request_end(&client.buffer) {
                                let action = match HttpRequest::parse_request(&client.buffer[..end])
                                {
                                    ...
                                    Ok(request) => {
                                        // => parsing successful, next step: route handling
                                        let host = request
                                            .headers
                                            .get("Host")
                                            .map(|h| h.as_str())
                                        ...
                                        match resolve_route(
                                            ...
                                            None => HttpResponseError::new_err_response(
                                                404,
                                                "Not Found",
                                            ), // => no matching route found -> bytes 404 response
                                            Some((server, route)) => {
                                                if request.body.len() > server.client_max_body_size
                                                {
                                                    HttpResponseError::new_err_response_with_pages(
                                                        413,
                                                        "Payload Too Large",
                                                        &server.error_pages,
                                                    )
                                                    ...
                                                    match request
                                                        .execute_route(route, server, client)
                                                    {
                                                        RouteAction::Immediate(response) => {
                                                            response
                                                        }
            ...
            let response = if result < 0 {
                ...
                match client.cgi_output_path.as_ref() {
                    Some(path) => match std::fs::read(path) {
                        Ok(cgi_output) => build_http_response_from_cgi_output(&cgi_output),
                        Err(_) => {
                            HttpResponseError::new_err_response(500, "Failed to read CGI output")
                        }
                    },
        ...
        let timed_out_fds: Vec<i32> = clients
            .iter()
        ...
        for timed_fd in &timed_out_fds {
            if let Some(client) = clients
                .iter_mut()
                .find(|c| c.socket.as_raw_fd() == *timed_fd)
            {
                let _ = client.socket.write_all(b"HTTP/1.1 408 Request Timeout\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
            }
            unsafe {
                epoll_ctl(epoll_fd, EPOLL_CTL_DEL, *timed_fd, std::ptr::null_mut());
            }
